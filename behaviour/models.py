...
class TripInformation(models.Model):
	...
	# Returns the string representation of the model.
	def __unicode__(self):              # __str__ on Python !=2
		return str(self.id) + " @ " + str(self.firsttimestamp) + " : " + str(self.device_id)

# Model with only the points
class TripCalculations(models.Model):
	# Regular Fields
	tripid = models.ForeignKey(
		TripInformation,on_delete=models.CASCADE)
	...
	# Returns the string representation of the model.
	def __unicode__(self):              # __str__ on Python !=2
		return str(self.tripid) + " @ " + str(self.city) + "(" + str(self.country) + ")"